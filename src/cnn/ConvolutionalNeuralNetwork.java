...
public class ConvolutionalNeuralNetwork {
	...
	public double test(Dataset testSet, boolean verbose) {
		...
		if (verbose) {
			...
			JSONArray json = null;
			try {
				json = (JSONArray) parser.parse(finalweights(weight));
			} catch (ParseException e) {
				e.printStackTrace();
			}
			FileWriter file = null;
			try {
				file = new FileWriter("C:/Users/shivy/OneDrive/Documents/Minor Project - CNN Model/imgclf/bin/output.json");
			} catch (IOException e) {
				e.printStackTrace();
			}
			try {
	...
	private String finalweights(String weight) {

		for (FullyConnectedLayer fcLayer : fullyConnectedLayers) {
			weight = fcLayer.finalweight(weight);
		}
		return weight;
	}
	@Override
	...
	private double[] labelToOneOfN(String label) {
		...
		//System.out.println(correctOutput[classes.indexOf(label)]);
